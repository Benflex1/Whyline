...
The release workflow checks out the tag ref, verifies that `package.json` is the version source of truth, runs the normal checks, creates and accepts one npm tarball, and uploads that exact `.tgz`. The protected release job downloads that artifact and publishes the downloaded file; it does not build or pack again.
