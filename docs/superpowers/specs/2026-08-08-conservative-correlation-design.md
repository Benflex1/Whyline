...
cheap deterministic ranking and bounded candidate set
  ↓
full evidence extraction for selected refs only
  ↓
...
The historical-commit-anchored repository match is conditional: a target commit
reference by itself does not establish repository identity. It represents a
candidate that may be evaluated because a current-repository object anchor exists;
it can reach `strong` only when the same session also supplies qualifying
structured patch overlap. A patch overlap with completely unknown repository
identity cannot become a selected match by itself.
...
- A session with no positive repository context may enter bounded extraction when
  its summary contains a safely resolved target commit reference or another
...
| --- | ---: | --- |
| `exact-target-commit-reference` | +10 | Only a uniquely resolved current-repository commit reference in credible context; in unknown context it participates in the historical conjunction rule, not alone. |
...
A stale nonmatching commit reference is represented, when useful, by the
...
- it has either a uniquely resolved target commit reference in credible context,
  or qualifying structured patch overlap.

For an unknown/deleted cwd, `exact-target-commit-reference` plus qualifying
structured patch overlap is required. Neither signal alone is enough.
...
signal is a structured patch/path-change signal. Repository context plus time,
...
boilerplate such as `return value;`. At least two distinctive matching line
fingerprints are required for a direct patch anchor. One matching line can still
be retained as non-strong supporting evidence, but cannot produce `strong`.

### Operation-aware sides

- `update` with `unified-diff`: compare Codex added-line fingerprints to Git
  added-line fingerprints.
...
The normalized adapter change carries bounded numeric unified-diff hunk ranges;
these ranges contain no source text and are used only for divergence comparison.
...
1. resolved target commit anchor;
...
- `complete`: discovery finished, no material discovery limitations exist, no
  ...
  extracted, and no material candidate evidence limitation remains;
...
- available source, zero refs: `none` with `empty-readable-store` limitation;
- available source with limited/unreadable portions: `limited` coverage and a
  result that cannot assert uniqueness;
- unavailable or unreadable source: `unavailable`.
...
repository candidates, or compaction/rollback/abort records that occur after the
last relevant evidence or before any reliable direct evidence.
...
4. Zero `strong` candidates yields `none`. A lone plausible candidate remains in
   ...
   not surfaced.
5. No readable/usable Codex history yields `unavailable`.
...
- Add normalized bounded numeric patch hunk ranges to `AgentPatchChange` for
  operation-aware divergence. The adapter continues to expose only supported
  structured patch results, fingerprints, paths, and diagnostics.
...
1. current repository plus exact target SHA → strong;
2. current repository plus two-line structured overlap → strong;
3. stale/rebased SHA plus current distinctive patch overlap → strong;
4. abbreviated target reference uniquely resolving versus ambiguous → only the
   unique reference contributes;
5. exact SHA with unknown repository context → not selected by SHA alone;
6. unknown/deleted cwd plus exact SHA plus matching patch → historical strong;
...
15. update, add, and delete operation overlap;
...
30. omitted eligible candidate or unresolved repository candidate → no matched;
31. empty readable store → none, not unavailable;
32. unreadable/unavailable source → unavailable.
